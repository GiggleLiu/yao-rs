...
def backend_report(directory):
    ...
    for path in sorted(directory.glob("*t-run*-rust.json")):
        ...
        for row in json.loads(path.read_text()):
            values[(threads, row["id"], row["backend"])].append(row["estimates"]["median"]["point_estimate"])
    ...
    summary = [dict(threads=t, id=case, backend=backend, median_ns=statistics.median(samples),
                    min_run_median_ns=min(samples), max_run_median_ns=max(samples), runs=len(samples))
               for (t, case, backend), samples in sorted(values.items())]
    (directory / "summary.json").write_text(json.dumps(summary, indent=2)+"\n")
    lookup = {(r["threads"], r["id"], r["backend"]): r for r in summary}
    lines = ["# CPU backend baseline", "", "Generated from raw Criterion and BenchmarkTools samples in this directory.", "",
             ...
             "| --- | --- | ---: | ---: | ---: | ---: |"]
    for row in summary:
        if row["backend"] != "native": continue
        other = lookup.get((row["threads"],row["id"],"julia"))
        if other is None: raise ValueError(f"Missing Julia measurement for {row['id']}")
        r,j=row["median_ns"],other["median_ns"]
        lines.append(f"| {row['threads']} | {row['id']} | {r/1000:.3f} | {j/1000:.3f} | {j/r:.2f} | {errors[row['id']]:.2e} |")
    lines += ["", "## Tensor and extension phases", "", "Each row uses the named API boundary. `tenferro_from_arrays` includes conversion, automatic planning, execution and output conversion; `omeinsum` includes its conversion/planning/execution. `tenferro_warm` uses an already prepared plan. These are independent planning policies, not a fixed-tree comparison.", "",
              "| Threads | Case | Phase | Median µs | Range of run medians µs |", "| --- | --- | --- | ---: | ---: |"]
    for row in summary:
        if row["backend"] in ("native","julia"): continue
        lines.append(f"| {row['threads']} | {row['id']} | {row['backend']} | {row['median_ns']/1000:.3f} | {row['min_run_median_ns']/1000:.3f}–{row['max_run_median_ns']/1000:.3f} |")
    lines += ["", "Raw confidence intervals and samples are in `*-rust.json`; Julia trial samples are in `*-julia.json`. Memory logs report instrumented Rust allocations per phase and whole-process peak RSS separately. Timings from the allocation instrument are diagnostic only. See `metadata.json` and pinned manifests for reproducibility.", ""]
